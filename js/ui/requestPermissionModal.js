...
import { getPermissionTypes, addSolicitud, uploadFile, updateNotificationCount } from '../dataController.js'; // Añadido updateNotificationCount
...
export function initializeRequestPermissionModal() {
    ...
    if (!requestPermissionModal) {
        console.error("ERROR - RequestPermissionModal: Modal #request-permission-modal no encontrado. ABORTANDO INICIALIZACIÓN.");
        return;
    }
    ...
    if (closeButton) {
        ...
        console.warn("WARN - RequestPermissionModal: Botón de cierre (.close-button) no encontrado en el modal principal.");
    }
    console.log("[DEBUG - RequestPermissionModal] Modal principal de solicitud de permiso inicializado.");
}
...
async function _initializeInternalDOMElements() {
    if (permissionRequestForm) { // Si ya se inicializaron
        console.log("[DEBUG - RequestPermissionModal] Elementos internos ya inicializados.");
        return true;
    }

    console.log("[DEBUG - RequestPermissionModal] Inicializando elementos internos del modal por primera vez...");
    ...
    if (!permissionRequestForm || !requestAgentSelect || !requestTypeSelect ||
        !requestStartDateInput || !requestEndDateInput || !requestCommentsInput || !requestAttachmentInput) {
        console.error("ERROR - RequestPermissionModal: Fallo al encontrar elementos DOM internos cruciales. Verifique IDs en index.html.");
        const missing = [];
    ...
    console.log("[DEBUG - RequestPermissionModal] Elementos internos y listeners inicializados completamente.");
    return true;
}
...
export function showRequestPermissionModal() {
    ...
    if (!requestPermissionModal) {
        console.error("[ERROR - RequestPermissionModal] Modal de solicitud de permiso no inicializado. No se puede mostrar.");
        return;
    }
    const initialized = _initializeInternalDOMElements();
    if (!initialized) {
        console.error("[ERROR - RequestPermissionModal] Falló la inicialización de elementos internos. No se muestra el modal.");
        return;
    }
    ...
    console.log("[DEBUG - RequestPermissionModal] Modal de solicitud de permiso visible. (display: flex)");
}
...
export function hideRequestPermissionModal() {
    ...
    if (requestPermissionModal) {
        ...
        console.log("[DEBUG - RequestPermissionModal] Modal de solicitud de permiso oculto. (display: none)");
    }
}
...
function populateAgentSelectForRequest() {
    ...
    if (!user || !agents || agents.length === 0) {
        console.warn("[DEBUG - RequestPermissionModal] No hay usuario o agentes disponibles para poblar el selector de solicitud.");
    ...
    if (user.role === 'admin') {
        ...
        console.log("[DEBUG - RequestPermissionModal] Selector de agente para solicitud poblado (Admin).");
    } else { // Rol 'guard'
        ...
        if (userAgent) {
            ...
            console.warn("[DEBUG - RequestPermissionModal] Agente del guardia no encontrado al poblar selector de solicitud.");
        }
        requestAgentSelect.disabled = true;
        console.log("[DEBUG - RequestPermissionModal] Selector de agente para solicitud poblado (Guardia).");
    }
...
async function handleSubmitRequest(event) {
    ...
    if (!requestAgentSelect || !requestTypeSelect || !requestStartDateInput || !requestEndDateInput || 
        !requestCommentsInput || !requestAttachmentInput) {
        console.error("ERROR - RequestPermissionModal: Algunos inputs del formulario no encontrados.");
        displayMessage("Error interno: Algunos campos del formulario no están listos. Recargue.", "error");
        return;
    }
    ...
    const requestData = {
        ...
        attachments: uploadedFileUrl ? [{ url: uploadedFileUrl, name: requestAttachmentInput.files[0]?.name || 'adjunto' }] : []
    };

    if (!requestData.agentId || !requestData.typeId || !requestData.startDate || !requestData.endDate) {
