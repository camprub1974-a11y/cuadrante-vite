...
let modal, form, closeButton, targetAgentSelect, requesterDateInput, targetDateInput, requesterShiftTypeDisplay, targetShiftTypeDisplay;
...
function populateAgentSelector() {
    ...
    agents.forEach(agent => {
        if (String(agent.id) !== String(user.agentId)) { // Ensure string comparison for agentId
...
function findShiftTypeForDate(agentId, date) {
    ...
    for (const weekKey in schedule.weeks) {
        for (const dayKey in schedule.weeks[weekKey].days) {
            ...
            if (day.date === targetDateString) { // Compare date strings
                // Find the shift for the specific agent on this day
                const shiftEntry = Object.values(day.shifts).find(s => String(s.agentId) === String(agentId));
...
async function handleFormSubmit(event) {
    ...
    try {
        ...
        if (!requestData.targetAgentId || requestData.requesterShiftType === '---' || requestData.targetShiftType === '---') {
