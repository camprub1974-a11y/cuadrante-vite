...
let pdfOptionsModal, pdfConfirmModal, pdfOptionsForm, generatePdfBtn, closeOptionsBtn, closeConfirmBtn, cancelConfirmBtn, confirmDownloadBtn;
let periodMonthRadio, periodRangeRadio, monthSelector, rangeSelector, pdfYearSelect, pdfMonthSelect, pdfStartDate, pdfEndDate, pdfAgentContainer, pdfAgentSelect, pdfFilename;
let summaryContent;
...
export function initializePdfGenerator() {
    ...
    if (!generatePdfBtn || !pdfOptionsModal || !pdfConfirmModal) {
        console.warn("Elementos para el generador de PDF no encontrados. La funcionalidad no estará disponible.");
        return;
    }
...
function populateYearSelector() {
    ...
    let options = '';
    for (let i = currentYear - 5; i <= currentYear + 1; i++) { // Rango de años ampliado un poco
...
function handleOptionsSubmit(event) {
    ...
    if (periodMonthRadio.checked) {
        ...
        if (!startDateValue || !endDateValue) {
            displayMessage("Por favor, selecciona tanto la fecha de inicio como la de fin para el rango personalizado.", "warning");
            return;
        }
        ...
        if (!isValid(rawStartDate) || !isValid(rawEndDate)) {
            displayMessage("Las fechas seleccionadas para el rango no son válidas. Formato esperado: AAAA-MM-DD.", "warning");
            console.error("Fechas inválidas detectadas después de parseISO:", startDateValue, endDateValue);
            return;
        }
    ...
    if (!isValid(rawStartDate) || !isValid(rawEndDate)) { // Doble chequeo, aunque las validaciones anteriores deberían cubrirlo
    ...
    const agentIds = allAgents ? null : (user.role === 'admin' ? [pdfAgentSelect.value] : [user.agentId]);
...
function showConfirmationModal() {
    ...
    if (pdfGenerationData.allAgents) {
        ...
        if (user.role === 'admin' && pdfGenerationData.agentIds && pdfGenerationData.agentIds.length > 0) {
            const agentName = availableAgents.get().find(a => a.id === pdfGenerationData.agentIds[0])?.name;
...
async function executePdfGeneration() {
    ...
    try {
        ...
        if (result.data && result.data.pdfBase64) {
            ...
            throw new Error(result.data?.message || "La función no devolvió un PDF o hubo un error desconocido en el backend.");
        }
    } catch (error) {
        console.error("Error al llamar a la función de generar PDF:", error);
        // HttpsError tiene propiedades 'code' y 'details'
        if (error.code && error.details) {
            displayMessage(`Error (${error.code}): ${error.message}. Detalles: ${JSON.stringify(error.details)}`, "error");
        } else {
