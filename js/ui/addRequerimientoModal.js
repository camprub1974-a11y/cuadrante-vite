...
export function initializeAddRequerimientoModal() {
    ...
    addRequerimientoModal.querySelector('.close-button').addEventListener('click', closeAddRequerimientoModal);
...
async function handleAddRequerimientoSubmit(event) {
    ...
    if (!reportId) {
        displayMessage('Error: ID de parte de servicio no encontrado para añadir requerimiento.', 'error');
        return;
    }
    ...
    if (!hora || !requirente || !motivo) {
        displayMessage('Por favor, rellena todos los campos obligatorios (Hora, Requirente, Motivo).', 'warning');
        return;
    }
