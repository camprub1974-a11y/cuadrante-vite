...
export function initializeStatsRenderer() {
    ...
    console.log("[DEBUG - StatsRenderer] Suscrito a los átomos scheduleData, selectedAgentId, selectedYear, selectedMonthId.");
}
...
async function renderStats() {
    ...
    if (currentAgentId === 'all' || !currentAgentId) {
        statsContainer.innerHTML = '<p class="info-message">Selecciona un agente individual para ver sus estadísticas de turnos.</p>';
        if (myChart) {
            myChart.destroy();
            myChart = null;
        }
    ...
    if (currentStatsRange === 'current_month') {
        ...
        if (!currentMonthSchedule || !currentMonthSchedule.weeks) {
            statsContainer.innerHTML = '<p class="info-message">Cargando datos del cuadrante para el mes actual...</p>';
        ...
        for (const weekKey in currentMonthSchedule.weeks) {
            ...
            if (week && week.days) {
                for (const dayKey in week.days) {
                    ...
                    if (day.isCurrentMonth) { // Solo días que pertenecen al mes actual
        ...
        statsContainer.innerHTML = '<p class="info-message">Calculando estadísticas anuales. Esto puede tardar unos segundos...</p>';
        if (myChart) { myChart.destroy(); myChart = null; }
        ...
        if (annualScheduleCache[currentYear]) {
            ...
            for (let i = 0; i < 12; i++) {
                const monthName = formatDate(new Date(currentYear, i, 1), 'MMMM', { locale: 'es' }).toLowerCase();
                ...
                try {
                    ...
                    console.warn(`[DEBUG - StatsRenderer] No se pudo cargar el cuadrante para ${monthId}:`, error.message);
                }
            }
            if (!hasAnyData) { // [MODIFICACIÓN] Si no se cargó NINGÚN día para el año
                statsContainer.innerHTML = '<p class="info-message">No hay datos de cuadrante disponibles para el año completo.</p>';
                return;
            }
    ...
    if (!scheduleToAnalyzeDays || scheduleToAnalyzeDays.length === 0) {
        statsContainer.innerHTML = '<p class="info-message">No hay datos suficientes para generar estadísticas con el rango seleccionado.</p>';
        return;
    }
    ...
    console.log(`[DEBUG - StatsRenderer] Estadísticas para agente ${currentAgentId} y rango ${currentStatsRange} renderizadas.`, agentStats);
}
 ...
 */
function calculateAgentStats(daysToAnalyze, agentId) { // [MODIFICACIÓN] Recibe los días directamente
    const stats = {
        totalDays: 0,
        totalHours: 0,
        shiftsCount: { 'M': 0, 'T': 0, 'L': 0, 'V': 0, 'PR': 0, 'AP': 0, '-': 0 },
        hoursByShift: { 'M': 0, 'T': 0 },
        weekdays: { 'Lun': { M: 0, T: 0 }, 'Mar': { M: 0, T: 0 }, 'Mié': { M: 0, T: 0 }, 'Jue': { M: 0, T: 0 }, 'Vie': { M: 0, T: 0 }, 'Sáb': { M: 0, T: 0 }, 'Dom': { M: 0, T: 0 } }
    };

    if (!daysToAnalyze || daysToAnalyze.length === 0) return stats;

    for (const day of daysToAnalyze) { // [MODIFICACIÓN] Iterar directamente sobre los días
        if (!day || !day.shifts) continue;
        
        const agentShift = Object.values(day.shifts).find(shift => String(shift.agentId) === String(agentId));
...
function renderStatsChart(stats) {
    ...
    const labels = ['Turno Mañana', 'Turno Tarde', 'Días Libres', 'Vacaciones', 'Asuntos Propios', 'Permisos Retribuidos'];
    const data = [
        stats.shiftsCount.M,
        stats.shiftsCount.T,
        stats.shiftsCount.L,
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            ...
                            const initialMap = {
                                'Turno Mañana': 'M', 'Turno Tarde': 'T', 'Días Libres': 'L',
                                'Vacaciones': 'V', 'Asuntos Propios': 'AP', 'Permisos Retribuidos': 'PR'
                            };
