...
export async function findAndRenderCurrentOrder() {
    ...
    try {
        ...
        if (!orderToDisplay) {
            orderToDisplay = activeOrders.find(order => 
                order.status === 'assigned' && formatDate(new Date(order.service_date), 'yyyy-MM-dd') === todayString
            );
        }
        if (!orderToDisplay) {
        ...
        currentOrderContent.innerHTML = '<p class="error-message">No se pudo cargar el servicio actual.</p>';
...
function renderCurrentOrderWidget(order) {
    ...
    const buttonTitle = isServiceToday ? 'Iniciar el parte de servicio' : 'Solo se puede iniciar el servicio en la fecha programada';

    const assignedAgentsNames = (order.assigned_agents || []).map(id => getAgentName(id)).join(', ');
