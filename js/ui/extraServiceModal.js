...
export function showExtraServiceModal(mode, date, serviceData = null) {
    ...
    if (mode === 'edit' && serviceData) {
        ...
        // Si el admin edita, guardamos el agentId original
...
async function handleFormSubmit(event) {
    ...
    const serviceDataPayload = {
        ...
        // Si el admin edita, mantenemos el agentId y userId originales del servicio
    ...
    try {
        ...
        document.dispatchEvent(new CustomEvent('extraServiceUpdated'));
    } catch (error) {
...
async function handleDeleteService() {
    if (!currentService || !currentService.id || !confirm('¿Estás seguro de que quieres eliminar este servicio?')) {
        return;
    }
    showLoading();
    try {
