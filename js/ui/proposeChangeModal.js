...
import { currentUser, availableAgents, selectedMonthId } from '../state.js';
import { displayMessage, showLoading, hideLoading } from './viewManager.js';
import { addShiftChangeRequest, updateNotificationCount, getScheduleForMonth } from '../dataController.js';
...
export function openProposeChangeModal(data) {
  ...
  const dateObject = requesterShiftDate ? parseISO(requesterShiftDate) : null;
...
async function fetchAndDisplayTargetShift() {
  ...
  // Obtener los datos del cuadrante para el mes actual
  let schedule = null;
  const currentMonthId = selectedMonthId.get(); // Obtener ID del mes desde el estado
  if (currentMonthId) {
    try {
      showLoading('Buscando turno...'); // Muestra indicador de carga
      schedule = await getScheduleForMonth(currentMonthId); // Llama a dataController
    } catch (error) {
      console.error("Error al obtener datos del cuadrante:", error);
      targetShiftInfo.textContent = 'Error al cargar cuadrante.';
  ...
  // El resto de la lógica para buscar el turno se queda igual...
  for (const weekKey in schedule.weeks) {
    for (const dayKey in schedule.weeks[weekKey].days) {
      ...
      if (day.date === targetDate) { 
        if (day.shifts) { // Asegurarse de que 'shifts' existe
  ...
  // ... (resto de la lógica para manejar foundShift, submitButton.disabled, etc.) ...
  ...
  if (foundShift) {
    ...
    targetShiftInfo.style.color = 'var(--color-success)'; 
