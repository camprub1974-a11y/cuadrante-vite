  ...
  document.querySelectorAll('.calendar-day').forEach((dayElement) => {
    ...
    if (userProfile && userProfile.role === 'admin') {
      dayElement.addEventListener('click', (event) => {
        // LOG 2: ¿Se ha detectado el clic en un día?
        console.log('2.  MOUSE CLICK DETECTADO en un día del calendario.');

        // LOG 3: ¿Se confirma el rol de admin en el momento del clic?
        console.log('3. 🕵️‍♂️ Rol de ADMIN confirmado. Procesando clic para edición.');
        ...
        if (dayData?.shifts) {
          ...
          if (shiftEntry) {
            currentShiftType = shiftEntry.shiftType;
          }
        }

        const agent = availableAgents.get().find(a => String(a.id) === String(agentIdToEdit));
        const agentName = agent ? agent.name : agentIdToEdit;

        const shiftDataForModal = {
            monthId: selectedMonthId.get(),
            weekKey: weekKey,
            dayKey: dayKey,
            agentId: agentIdToEdit,
            agentName: agentName,
            dateString: actualDayDate,
            currentShiftType: currentShiftType
        };

        // LOG 4: ¿Se están preparando los datos correctos antes de llamar al modal?
        console.log('4. ➡️ Preparando para abrir modal con los siguientes datos:', shiftDataForModal);

        openShiftModal(shiftDataForModal);
      });
