...
import { renderContext, availableAgents, currentUser, selectedAgentId, setAgent } from '../state.js';
        ...
        if (userProfile && userProfile.role === 'admin') {
             dayElement.addEventListener('click', (event) => {
                ...
                if (String(agentIdToEdit) === 'all' || !agentIdToEdit) {
                    displayMessage("Por favor, selecciona un agente individual para editar su turno.", "info");
                    return;
                }
                ...
                if (dayData?.shifts) {
                    const shiftEntry = Object.entries(dayData.shifts).find(([,s]) => String(s.agentId) === String(agentIdToEdit));
                    if(shiftEntry) {
                ...
                openEditShiftModal(weekKey, dayKey, agentIdToEdit, currentShiftType, actualDayDate, dayElement, existingShiftKey);
            });
            ...
            dayElement.addEventListener('click', (event) => {
                ...
                if (dayData?.shifts) {
                    const shiftEntry = Object.entries(dayData.shifts).find(([,s]) => String(s.agentId) === String(agentIdFromCell));
...
export function renderAvailabilityView(context) {
    ...
    const { userProfile, scheduleData: currentScheduleData, selectedAgentId: currentSelectedAgentId } = context;
    ...
    if (!viewContainer) {
        console.error("No se encontró el contenedor para la vista de calendario:", viewContainerSelector);
        return;
    }
    ...
    if (userProfile.role === 'guard' && String(currentSelectedAgentId) !== String(userProfile.agentId)) {
    ...
    weekDays.forEach(dayName => { html += `<div class="day-name">${dayName}</div>`; });
    html += '</div>';
        ...
        .forEach(weekKey => {
            ...
            for (let i = 0; i <= 6; i++) {
                ...
                if (currentSelectedAgentId !== 'all') {
                    agentShifts = Object.values(day.shifts || {}).filter(shift => String(shift.agentId) === String(currentSelectedAgentId));
                } else {
                ...
                const extraServicesForDay = currentScheduleData.extraServices && currentScheduleData.extraServices[day.date] ? currentScheduleData.extraServices[day.date] : [];
                const agentExtraServices = extraServicesForDay.filter(service => String(service.agentId) === String(currentSelectedAgentId));

                let extraServiceTitles = [];
                if (agentExtraServices.length > 0) {
                    agentExtraServices.forEach(service => {
                        extraServiceTitles.push(`${EXTRA_SERVICE_TYPES[service.type].name} (${service.hours}h)`);
                        ...
                        extraServiceBadge.textContent = EXTRA_SERVICE_TYPES[service.type].name.charAt(0).toUpperCase();
                ...
                const isMarkedDate = markedDatesForMonth.some(markedDate => day.date === formatDate(markedDate.date, 'yyyy-MM-dd'));
                const dayTitle = extraServiceTitles.length > 0 ? `title="${extraServiceTitles.join(', ')}"` : '';
