...
import { selectedAgentId, selectedMonthId, scheduleData, renderContext, availableAgents } from './state.js';
import { showLoading, hideLoading } from './ui/viewManager.js';
import { updateSeasonalNote } from './logic.js';
import { formatDate, getShiftDisplayText, getMonthNumberFromName, parseDateString } from './utils.js';
...
function generateTableHtml(daysData, agentsData, isFirstTable = true) {
    ...
    daysData.forEach(day => tableHtml += `<th class="day-number-header day-column">${day.number}</th>`);
    tableHtml += '</tr><tr>';
...
function displayPrintScheduleView(printPersonIdToFilter, forceAllAgentsForPrint = false) {
    ...
    allDaysInPrintTable.sort((a, b) => parseDateString(a.date).getTime() - parseDateString(b.date).getTime());
    ...
    if (allDaysInPrintTable.length > 0) { 
        printHtml += generateTableHtml(allDaysInPrintTable.slice(0, daysPerTable), agentsToRender, true);
        if (allDaysInPrintTable.length > daysPerTable) {
            printHtml += `<div style="page-break-before: always;"></div>`; 
            printHtml += generateTableHtml(allDaysInPrintTable.slice(daysPerTable), agentsToRender, false);
        }
    } else {
...
export function handlePrintButtonClick() {
    ...
    printOptionsModal.style.cssText = 'display:flex;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);justify-content:center;align-items:center;z-index:2000;';
