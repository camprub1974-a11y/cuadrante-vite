...
// AHORA, MODIFICA LA FUNCIÓN 'createRegistro' existente
export const createRegistro = onCall(
  {
    region: 'us-central1',
    memory: '1GB',
  ...
  async (request) => {
    // ... (toda tu validación y lógica existente se mantiene)
    // Buscamos el nuevo campo 'linkedTaskId' que enviaremos desde el cliente
    const { documentType, data: registroData, linkedTaskId } = request.data;
    // ...

    try {
      // ... (tu lógica para generar el número de registro se mantiene)
      const registration_number = await db.runTransaction(async (transaction) => {
        // ...
        return `${prefix}${String(nextNumber).padStart(4, '0')}`;
      });
      
      // ... (tu lógica de creación del documento en Firestore)
      const docRef = await db.collection('registros').add(newDocument);
      
      // --- INICIO DE LA MODIFICACIÓN ---
      // Si se creó el registro en el contexto de una tarea, actualizamos la tarea.
      if (linkedTaskId) {
        const taskRef = db.collection('tareas').doc(linkedTaskId);
        await taskRef.update({
          status: 'finalizada',
          completedAt: admin.firestore.FieldValue.serverTimestamp(),
          resolutionDocumentId: docRef.id, // ID del nuevo registro
          resolutionDocumentNumber: registration_number // Número del nuevo registro
        });
        logger.info(`Tarea ${linkedTaskId} finalizada y vinculada al registro ${docRef.id}.`);
      }
      // --- FIN DE LA MODIFICACIÓN ---

      // ... (tu lógica de generación de PDF se mantiene)
