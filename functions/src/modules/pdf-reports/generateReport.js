...
async function generateExtraordinaryServicesPdfReport(agentId, startDate, endDate) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            doc.text(`Periodo del Informe: ${startDate ? format(parseISO(startDate), 'dd/MM/yyyy', { locale: es }) : 'Inicio'} al ${endDate ? format(parseISO(endDate), 'dd/MM/yyyy', { locale: es }) : 'Fin'}`);
            doc.moveDown();
            ...
            services.forEach(service => {
                const serviceDateFormatted = format(parseISO(service.date), 'dd/MM/yyyy HH:mm', { locale: es });
                const serviceTotal = service.price * service.hours; // Calcular el total por servicio
                totalAcumuladoPeriodo += serviceTotal;

                doc.text(serviceDateFormatted, 50, doc.y, { width: 100, align: 'left' })
            ...
            doc.moveDown();
            doc.font('Helvetica-Bold').fontSize(14).text(`Total Acumulado del Periodo: ${totalAcumuladoPeriodo.toFixed(2)}`, { align: 'right' });
